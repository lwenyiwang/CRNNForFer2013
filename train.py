...
trainset = CK(path = './data/train_data.h5', transform=transform_train)
trainloader = torch.utils.data.DataLoader(trainset, batch_size=opt.bs, shuffle=True, num_workers=1)
testset = CK(path = './data/dev_data.h5', transform=transform_test)
...
def test(epoch):
    ...
    for batch_idx, (inputs, targets) in enumerate(testloader):
        ...
        tloss=PrivateTest_loss / (batch_idx + 1)
    # Save checkpoint.
    Test_acc = 100.*correct/total
    Loss_list.append(tloss)
    Accuracy_list.append(Test_acc/100.0)
...
for epoch in range(start_epoch, total_epoch):
    train(epoch)
    test(epoch)
epo=range(start_epoch, total_epoch)
y1 = Accuracy_list
y2 = Loss_list
file= open('VGGRNN-valloss.txt', 'w')
for i in Loss_list:
    file.write(str(i))
    file.write('\n')
file.close()
file= open('VGGRNN-valaccuracy.txt', 'w')
...
plt.savefig("CNN426_accuracy_loss_last.png")
